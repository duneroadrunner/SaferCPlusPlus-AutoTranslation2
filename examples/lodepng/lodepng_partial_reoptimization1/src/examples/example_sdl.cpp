...
int show(const mse::mstd::string&  caption, mse::lh::TLHNullableAnyRandomAccessIterator<const unsigned char>  rgba, unsigned w, unsigned h)
{
  ...
  for(unsigned x = 0; x + jump - 1 < w; x += jump)
  {
    ...
    MSE_LH_SUPPRESS_CHECK_IN_XSCOPE Uint32* bufp;
    MSE_LH_SUPPRESS_CHECK_IN_XSCOPE bufp = (Uint32 *)scr->pixels + (y * scr->pitch / 4) / jump + (x / jump);
    *bufp = 65536 * r + 256 * g + b;
  }

  //pause until you press escape and meanwhile redraw screen
  MSE_LH_SUPPRESS_CHECK_IN_XSCOPE SDL_Event event;
  int done = 0;
  while(done == 0)
  {
...
int showfile(mse::lh::TXScopeLHNullableAnyRandomAccessIterator<const char>  filename)
{
  ...
  mse::TNoradObj<unsigned int > w = 0/*auto-generated init val*/; mse::TNoradObj<unsigned int > h = 0/*auto-generated init val*/;
