...
{
    uh_epoll_data *s = (uh_epoll_data *)(malloc(sizeof *s));
...
{
    uh_epoll_data *l = (uh_epoll_data *)(malloc(sizeof *l));
...
{
    uh_epoll_data *t = (uh_epoll_data *)(malloc(sizeof *t));
                           ...
                           const char *port)
{
    ...
    *out_listen_fds_len = 0;
         ...
         result = result->ai_next)
    {
        ...
        (*out_listen_fds_len)++;
        *out_listen_fds = (int *)(realloc(*out_listen_fds, sizeof(**out_listen_fds) *
                                                       (*out_listen_fds_len)));
...
tprp_rv try_parse_request_path(session_t *session)
{
    ...
    session->request_path_len =
        (char *)(trailing_space) - (char *)((void *)session->request) - http_get_prefix_len;
    session->request_path = (char *)(malloc(session->request_path_len));
...
void rewrite_path(session_t *session)
{
    if (session->request_path_len == 1 && session->request_path[0] == '/')
    {
        ...
        session->request_path =
            (char *)(realloc(session->request_path, session->request_path_len));
...
void generate_response(session_t *session)
{
    ...
    // to the key.
    char *null_term_path = (char *)(malloc(session->request_path_len + 1));
    ...
    // a valid one
    content_t *content = (content_t *)(ht_get(content_ht, null_term_path));
    ...
    if (content == NULL)
    {
        ...
        session->response = (uint8_t *)(malloc(strlen(fof)));
    ...
    else
    {
        ...
        char *content_len = (char *)(malloc(content_len_strlen + 1));
        ...
        char *content_type;
        char *extension_separator = (char *)(strrchr(null_term_path, '.'));
        ...
        session->response = (uint8_t *)(malloc(session->response_len));
        check_error_null(session->response, "malloc session->response");
                      ...
                      struct FTW *ftwbuf)
{
    ...
    uint8_t *content_bytes = (uint8_t *)(malloc(sb->st_size));
    ...
    content_t *content = (content_t *)(malloc(sizeof *content));
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        {
            // content dir
            content_dir_path = (char *)(malloc(strlen(optarg) + 1));
        ...
        for (int i = 0; i < num_events; i++)
        {
            ...
            else if (fd_type == EPT_CONNECTION)
            {
                ...
                if (epoll_events[i].events & EPOLLIN)
                {
                    ...
                    ed->session.request =
                        (char *)(realloc(ed->session.request, ed->session.request_len));
